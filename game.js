...
function increaseScore() {
    // increase now our coounter when our flappy passes the pipes
...
function showEndMenu() {
    ...
    // This way we update always our highScore at the end of our game if we have a higher high scoree than the previous
    if (highScore < score) {
        highScore = score
    }
...
function loop() {
    ...
    // Draw Pipes
    ctx.fillStyle = '#333';
    ctx.fillRect(pipeX, -100, BIRD_WIDTH, pipeY);
    ctx.fillRect(pipeX, pipeY + PIPE_GAP, PIPE_WIDTH, canvas.height - pipeY);

    // now we would need to add an collision check to display our end-menu and end the game

    // the collisionCheck will return us true if we have a collisionCheck, otherwise false
    if (collisionCheck()) {
        endGame();
        return;
    }
